...
def export_to_xlsx(modeladmin, request, queryset):
    ...
    for row, obj in enumerate(queryset):
        ...
        for product in prod_tracker.values():
            data_row.append(product['qty'])
            data_row.append(product['price'])
            order_price_total += (product['qty'] * product['price'])
...
def order_pdf(obj):
    return format_html('<a href="{}" target="_blank">PDF</a>',
                       reverse('orders:invoice_pdf_url', args=[obj.id]))
